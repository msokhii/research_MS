...
Constuct_Sys_Blackbox := proc(Sys,Vars,params)
    ...
    Lin_BB := proc(point_::list(integer),p::prime)
        local A,T,subs_values,num_eqn,soln;
        uses LinearAlgebra:-Modular:
        global counter;
        counter := counter+1:
        # global bb_calls_ndsa,bb_calls_mrfi,t_ndsa_total,t_mrfi_total,bb_phase:
        # local t0, t_avg;
...
get_point_on_affine_line := proc(num_var::posint,alpha::list,beta_::list,
    ...
    return [seq(
        [alpha[np],
        seq((beta_[nv-1]*(alpha[np]-s1)+sigma_[nv]) mod p,
        nv = 2 .. num_var)],np = 1..T)]:
end proc:
...
MRFI := proc(B,num_vars::integer,num_eqn::integer,vars::list,p::integer)
    local i,j,k,s,Primes,direction,sigma_,num_eval,den_eval,u,mon,
          ...
          init_sigma,sampleCounts,mMax,tempG,ratReconVal,
          sigma_j,sigma_run,alphaVal,modulusTable,modulusPoly,
    ...
    tempR := rand(1..p-1):
    direction := [seq(tempR(),i=1..num_vars-1)]:
    init_sigma := Primes:
    sigma_ := []:
    num_eval := table(): 
    num_eval_n := table():
    ...
    od:
    print(convert(modulusTable,list)):
    ...
    while true do
        for j from jDone+1 to 2*Tcur do
            ...
            if lin_sys then
                ...
                for i from 1 to num_eqn do
                    if numerator_done[i] and denominator_done[i] then 
                        next: 
                    fi:
                    m_i := sampleCounts[i]:
                    ...
                    if not numerator_done[i] then
                        ...
                        num_eval[i][num_eval_n[i]] := tmpNum:
                    fi:
                    if not denominator_done[i] then
                        den_eval_n[i] := den_eval_n[i]+1:
                        den_eval[i][den_eval_n[i]] := tmpDen:
                    fi:
        ...
        jDone := 2*Tcur:
        print(convert(num_eval,list)):
        print(convert(den_eval,list)):
        print(nops(convert(num_eval,list))):
        print(nops(convert(den_eval,list))):
        quit;
        all_done := true:
        for k from 1 to num_eqn do
            ...
            # t_helper := time():
            lambda_num[k] := BMEA([seq(num_eval[k][s],s=1..num_eval_n[k])],p,Z):
        ...
        if common_den_flag then
            if not denominator_done[1] then
                # t_helper := time():
                lambda_den[1] := BMEA([seq(den_eval[1][s],s=1..den_eval_n[1])],p,Z):
                # t_bmea_total := t_bmea_total+(time()-t_helper):
            ...
            for k from 1 to num_eqn do
                ...
                # t_helper := time():
                lambda_den[k] := BMEA([seq(den_eval[k][s],s=1..den_eval_n[k])],p,Z):
                # t_bmea_total := t_bmea_total+(time()-t_helper):
    ...
    for k from 1 to num_eqn do
        ...
        num_mono[k] := temp:
        # t_helper := time():
        coeff_num[k] := Zippel_Transpose_Vandermonde_solver([seq(num_eval[k][s],s=1..terms_num[k])],
    ...
    if common_den_flag then
        ...
        den_mono[1] := temp:
        # t_helper := time():
        coeff_den[1] := Zippel_Transpose_Vandermonde_solver([seq(den_eval[1][s],s=1..terms_den[1])],
        ...
        for k from 1 to num_eqn do
            ...
            den_mono[k] := temp:
            # t_helper := time():
            coeff_den[k] := Zippel_Transpose_Vandermonde_solver([seq(den_eval[k][s],s=1..terms_den[k])],
