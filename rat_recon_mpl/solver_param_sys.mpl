...
(* 
Local var. catalog is a maple table that contains a list. 
For example: 
catalog["A1][1] is the name of the matrix. 
catalog["A1][2] contains a maple procedure that constructs the above matrix.
*)
	...
	catalog["A2"] := ["General Toeplitz",
			         proc(n) 
			         [Matrix(n,n,(i,j)->piecewise(i=j,t[0],j>i,r[j-i],s[i-j])),
			         Vector(n,fill=1)] 
			         end proc]:
	...
	catalog["A3"] := ["Hankel",
			         proc(n) 
			         [Matrix(n,n,(i,j)->h[i+j-2]),Vector(n,fill=1)] 
			         end proc]:
   
    (* Identical stats. as A1 with a bit more probes. *)
	catalog["A4"] := ["Banded Toeplitz",
                     proc(n)
                     local b:
                     b := 2:
			         [Matrix(n,n,(i,j)->`if`(abs(j-i)<=b,t[j-i],0)),
                     Vector(n,fill=1)]
                     end proc]:

    (* Not running. *)
	catalog["A5"] := ["Block Toeplitz with Toeplitz Blocks",
                     proc(n)
                     [Matrix(n^2,n^2,(i,j)->t[floor((i-1)/n)-floor((j-1)/n),
                     ((i-1) mod n)-((j-1) mod n)]),Vector(n^2,fill=1)]
                     end proc]:

	catalog["A6"] := ["Generic Tridiagonal",
                     proc(n) 
                     [Matrix(n,n,(i,j)->piecewise(i=j,a[i],j=i+1,u[i],i=j+1, 
                     l[j], 0)),Vector(n,i->i)] 
                     end proc]:

    (* Failing at n=5 *)
	catalog["B1"] := ["Sylvester Resultant",
                     proc(n)
                     [Matrix(2*n,2*n,
                     (i,j)->if i <= n then
                                if 0 <= j-i and j-i <= n then
                                    a[n-(j-i)]
                                else
                                    0
                                fi:
                            else
                                if 0 <= j-(i-n) and j-(i-n) <= n then
                                    b[n-(j-(i-n))]
                                else
                                    0
                                fi:
    ...
    catalog["B2"] := ["Grounded Weighted Grid Laplacian",
                     proc(q)
                     local N,L,i,j,u,w:
                     ...
                     for i from 1 to q do
                        for j from 1 to q-1 do
                            w := h[i,j]:
                            L[u(i,j),u(i,j)] := L[u(i,j),u(i,j)]+w:
                            L[u(i,j+1),u(i,j+1)] := L[u(i,j+1),u(i,j+1)]+w:
                            L[u(i,j),u(i,j+1)] := -w:
                            L[u(i,j+1),u(i,j)] := -w:
                        od:
                     od: 
                     ...
                     for i from 1 to q-1 do
                        for j from 1 to q do
                            w := v[i,j]:
                            L[u(i,j),u(i,j)] := L[u(i,j),u(i,j)]+w:
                            L[u(i+1,j),u(i+1,j)] := L[u(i+1,j),u(i+1,j)]+w:
                            L[u(i,j),u(i+1,j)] := -w:
                            L[u(i+1,j),u(i,j)] := -w:
                        od:
                     od:

                     [L[1..N-1,1..N-1],Vector(N-1,fill=1)]:
                     end proc
                     ]:

    (* This works well. n^2 x n^2 system. *)

    catalog["B3"] := ["2D Convection-Diffusion Finite Difference",
                 proc(n)
                                  local N,A,i,j,k,d,e,w,nn,s;
    ...
    (* Works well with a 64 bit prime. Fails for n=8. *)
                                                                                                                                                                                                                                                                              
    catalog["B4"] := ["RC Ladder Nodal Admittance",
                     proc(n)
                                      local A,i;
    ...
    (* Works well with a 64 bit prime. *)

    catalog["B5"] := ["Mass-Spring Chain Dynamic Stiffness",
                     proc(n)
                                      local A,i;
    ...
    catalog["B6"] := ["Birth-Death Markov Chain Reachability",
                     proc(n)
                                      local A,b,i;
    ...
    catalog["C1"] := ["B-spline",
                     ...
                     Sys := [x7 + x12 - 1, x8 + x13 - 1, 
                            x21 + x6 + x11 - 1,x1*y1 + x1 - x2, 
                            x11*y3 + x11 - x12, x16*y5 - x17*y5 - x17,
    ...
    catalog["C2"] := ["Small System Low Degree",
                     proc(n)
                     local Sys,vars,A,b:
...
(* In the below procedure, id is the catalog key. So "A1","A2",etc. *)

    Generate := proc(id,n)
        if not assigned(catalog[id]) then 
            error "Unknown id: %1",id 
        fi:
        catalog[id][2](n)
    end proc:
    ...
    Show := proc(id,n)
        ...
        r := Generate(id,n):
        r1,r2 := Dimension(r[1]):
        printf("%-4s %s   (%d x %d)\n",id,catalog[id][1],r1,r2);
        print(r[1]):  
        print(r[2]):
        NULL (* Maple's special NULL. This returns nothing. *)
    end proc:
...
(* Returns the entire catalog table. *)
...
(* Returns the actual parameters of the system. *)
...
ExpectedRawParams := proc(id::string,n::posint)
    ...
    if id = "A1" then
        ...
        return [seq(t[i],i=-2..2)]:
    elif id = "A5" then
        return [seq(seq(t[i,j],j=-(n-1)..n-1),i=-(n-1)..n-1)]:
    elif id = "A6" then
        return [r]:
    elif id = "B1" then
        return [seq(a[i],i=0..n),seq(b[i],i=0..n)]:
    elif id = "B2" then
        return [seq(seq(h[i,j],j=1..n-1),i=1..n),
                seq(seq(v[i,j],j=1..n),i=1..n-1)
               ]:
    elif id = "B3" then
        return [eps,cx,cy]:
    elif id = "B4" then
        return [s,seq(g[i],i=1..n-1),seq(c[i],i=1..n)]:
    elif id = "B5" then
        return [seq(k[i],i=1..n+1),seq(d[i],i=1..n)]:
    elif id = "B6" then
        return [seq(lambda[i],i=1..n),seq(mu[i],i=1..n)]:
    fi:
...
DB_system := proc(id::string,n::posint)
    local res,A,b,nn,Sys,pars0,newp,remap,Vars,i,j,params:
    ...
    nn := op(1, b):
    Vars := [seq(x||i, i=1..nn)]:
    params := [seq(y||i,i=1..nn)]:
    Sys := [seq(add(A[i,j]*Vars[j], j=1..nn) = b[i], i=1..nn)]:
    if id <> "C1" and id <> "C2" and id <> "C3" 
    and id <> "C4" and id <> "C5" then
        pars0 := ExpectedRawParams(id,n):
        pars0 := sort(pars0,(u,v)->lexorder(convert(u,string),convert(v,string))):
