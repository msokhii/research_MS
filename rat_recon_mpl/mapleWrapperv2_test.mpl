 libObj := "/cecm/home/mss59/Desktop/research_ServerMaple/inuse_cpp_routines/cpp_routines/testObj.so":
...
(* Discrete-log Ben-Or/Tiwari monomial recovery.  Returns 0 = OK,
   1 = FAIL (root not of monomial form), <0 = bad input.  expOut is the
   row-major T x n exponent matrix. *)

mGENMONO := define_external(
                          'cppGenMono',
                          T::integer[4],
                          roots::ARRAY(0..T-1, datatype=integer[8]),
                          n::integer[4],
                          omega::ARRAY(0..n-1, datatype=integer[8]),
                          q::ARRAY(0..n-1, datatype=integer[8]),
                          p::integer[8],
                          outLen::integer[4],
                          expOut::ARRAY(0..outLen-1, datatype=integer[8]),
                          RETURN::integer[4],
                          LIB=libObj
                          ):

(* Remove type checking from mGENMONO. *)
mGENMONO := subsop(1=(T,roots,n,omega,q,p,outLen,expOut),
                   op(mGENMONO)):

(* Maple wrapper.  roots_ are the Berlekamp-Massey generator roots, omega_
   the order-q_[j] evaluation points, vars the parameter names.  Returns the
   list of recovered monomials, or FAIL. *)

cppGenMono := proc(roots_::list, omega_::list, q_::list, p, vars::list)
    local T,n,i,j,rArr,oArr,qArr,eArr,rc:
    T := nops(roots_):
    n := nops(omega_):
    if T = 0 then
        return []:
    fi:
    if nops(q_) <> n or nops(vars) <> n then
        error "omega_, q_ and vars must have the same length.":
    fi:
    rArr := Array(0..T-1,datatype=integer[8]):
    for i from 1 to T do
        rArr[i-1] := roots_[i]:
    od:
    oArr := Array(0..n-1,datatype=integer[8]):
    qArr := Array(0..n-1,datatype=integer[8]):
    for j from 1 to n do
        oArr[j-1] := omega_[j]:
        qArr[j-1] := q_[j]:
    od:
    eArr := Array(0..T*n-1,datatype=integer[8]):
    rc := mGENMONO(T,rArr,n,oArr,qArr,p,T*n,eArr):
    if rc < 0 then
        error "cppGenMono external error code %1", rc:
    fi:
    if rc = 1 then
        return FAIL:
    fi:
    return [seq(mul(vars[j]^eArr[(i-1)*n+j-1],j=1..n),i=1..T)]:
end proc:
