...
for n_test from n_min to n_max do
    ...
    for run_idx from 1 to num_runs do
        counter := 0:
        num_lines := 0:
        bb_phase := "MRFI":
        bb_calls_ndsa := 0:
    ...
    od:
    t_mrfi_wall := (time()-t_mrfi_wall_start)/num_runs:
    t_ndsa_total      := t_ndsa_total/num_runs:
    t_mrfi_total      := t_mrfi_total/num_runs:
    t_cppNewton_total := t_cppNewton_total/num_runs:
    t_mqrfr_total     := t_mqrfr_total/num_runs:
    t_cppRR_total     := t_cppRR_total/num_runs:
    t_bmea_total      := t_bmea_total/num_runs:
    t_zippel_total    := t_zippel_total/num_runs:
    t_roots_total     := t_roots_total/num_runs:
    t_genmono_total   := t_genmono_total/num_runs:
    t_construct_total := t_construct_total/num_runs:
    t_points_total    := t_points_total/num_runs:
    t_seqeval_total   := t_seqeval_total/num_runs:
    t_normalize_total := t_normalize_total/num_runs:
    t_NDSA_comp       := t_NDSA_comp/num_runs:
    mrfi_calls := counter:
    if status = "OK" then
    ...
    if do_ffge then
        ...
        t_ffge := (time()-t_ffge_start)/num_runs:
...
report_path := "/cecm/home/mss59/Desktop/research_ServerMaple/rat_recon_mpl/timings/avgTime.txt":
