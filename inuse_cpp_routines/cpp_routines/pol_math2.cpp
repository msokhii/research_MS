...
/*
KARATSUBA MULTIPLICATION AND NEWTON REMAINDER FOR THE MODULAR POWERING.

polpowmod64s spends its life on one line: C := C^2 mod B.  Classically that is
one d^2 multiplication (polsqr64s) plus one d^2 division (poldiv64s) per bit of
the exponent, and at deg(B) = 5579 with a 62 bit prime the two together are the
entire root finding cost.  Below:

  polmulKAR64    Karatsuba multiplication, O(d^1.585), falling back to
                 polmul64s below POLKAR_CUTOFF where the ZFMA kernel wins.
  polrecipNEWT64 the power series inverse of rev(B) to precision deg(B),
                 by Newton iteration g <- g*(2 - rev(B)*g); computed ONCE per
                 powmod call, since B never changes inside it.
  polremNEWT64   remainder of a by monic B as two Karatsuba multiplications
                 against that precomputed inverse, replacing poldiv64s.
  polpowmodKAR64 the powering loop rebuilt on those three.  The base A has
                 degree <= 1 at both call sites (x and x+alpha), so the odd
                 bit steps are done directly in O(d).

polpowmod64s dispatches here when deg(B) >= POLKARPOW_CUTOFF and is unchanged
otherwise.  polsplit64s and polroots64s call polpowmod64s as before and see
identical results; only the time changes.  All arithmetic is add64b / sub64b /
mul64b on reduced representatives, so nothing here depends on recint.
*/

/* POLKAR_CUTOFF: below this, polmul64s's ZFMA kernel wins over splitting.
   POLKARPOW_CUTOFF: below this, the CLASSICAL powmod wins overall.  Each bit
   costs the classical loop one multiply plus one divide, and polDIVIP64 runs
   at the same ~1 product/cycle as polmul64s, so the Newton route (one square
   plus two Karatsuba multiplies) only pays once the Karatsuba multiply is
   under 2/3 of the classical one -- measured on this machine near degree 500
   (32 bit p) to 1000 (64 bit p).  400 is a compromise; the split tree spends
   little time between 400 and 1000 either way. */
#ifndef POLKAR_CUTOFF
#define POLKAR_CUTOFF    64
#endif
#ifndef POLKARPOW_CUTOFF
#define POLKARPOW_CUTOFF 400
#endif

// c[0..2n-2] := a[0..n-1] * b[0..n-1] mod p.  ws needs 8n+64 slots.
static void polmulKAR64(const LONG *a,const LONG *b,LONG *c,int n,
                        const LONG p,LONG *ws){
    int h,n1,i;
    LONG *as,*bs,*z1,*wnext;
    if( n<=POLKAR_CUTOFF ) {
        polmul64s((LONG*)a,(LONG*)b,c,n-1,n-1,p);
        return;
    }
    h  = n/2;
    n1 = n-h;                                 // n1 >= h
    as = ws; bs = ws+n1; z1 = ws+2*n1; wnext = ws+4*n1;
    polmulKAR64(a,b,c,h,p,wnext);             // z0 = a0*b0 in c[0..2h-2]
    c[2*h-1] = 0;
    polmulKAR64(a+h,b+h,c+2*h,n1,p,wnext);    // z2 = a1*b1 in c[2h..2n-2]
    for( i=0; i<n1; i++ ) {                   // as = a0+a1, bs = b0+b1
        as[i] = (i<h) ? add64b(a[i],a[h+i],p) : a[h+i];
        bs[i] = (i<h) ? add64b(b[i],b[h+i],p) : b[h+i];
    }
    polmulKAR64(as,bs,z1,n1,p,wnext);         // z1 = (a0+a1)(b0+b1)
    for( i=0; i<2*h-1; i++ ) z1[i] = sub64b(z1[i],c[i],p);
    for( i=0; i<2*n1-1; i++ ) z1[i] = sub64b(z1[i],c[2*h+i],p);
    for( i=0; i<2*n1-1; i++ ) c[h+i] = add64b(c[h+i],z1[i],p);
    return;
}

// pad a[0..da] and b[0..db] to a common length and multiply; c gets da+db+1
// coefficients.  ap/bp/ws are caller workspace: ap,bp of size n, ws of 8n+64,
// where n = max(da,db)+1.
static void polmulKARpad64(const LONG *a,int da,const LONG *b,int db,LONG *c,
                           const LONG p,LONG *ap,LONG *bp,LONG *ws){
    int n,i;
    n = (da>db?da:db)+1;
    for( i=0; i<n; i++ ) { ap[i] = (i<=da)?a[i]:0; bp[i] = (i<=db)?b[i]:0; }
    polmulKAR64(ap,bp,c,n,p,ws);
    return;
}

// g[0..prec-1] := inverse of v = rev(B) mod x^prec, B monic so v[0]=1.
// t and ws are workspace: t of size 6*prec+8, ws of 16*prec+64.
static void polrecipNEWT64(const LONG *B,int db,int prec,LONG *g,
                           const LONG p,LONG *t,LONG *ws){
    int k,k2,i,nv;
    LONG *t2,*vp,*gp;
    t2 = t+2*prec+2; vp = t+4*prec+4; gp = t+5*prec+6;
    g[0] = 1;
    k = 1;
    while( k<prec ) {
        k2 = 2*k; if( k2>prec ) k2 = prec;
        nv = (db+1<k2) ? db+1 : k2;
        for( i=0; i<k2; i++ ) vp[i] = (i<nv) ? B[db-i] : 0;   // rev(B) mod x^k2
        for( i=0; i<k2; i++ ) gp[i] = (i<k) ? g[i] : 0;
        polmulKAR64(vp,gp,t,k2,p,ws);                          // t = v*g
        t[0] = sub64b(2%p,t[0],p);                             // t = 2 - v*g
        for( i=1; i<k2; i++ ) t[i] = neg64s(t[i],p);
        polmulKAR64(gp,t,t2,k2,p,ws);                          // g*(2-v*g)
        for( i=0; i<k2; i++ ) g[i] = t2[i];
        k = k2;
    }
    return;
}

// a[0..da] := a mod B, deg returned.  B monic of degree db, Binv the inverse
// of rev(B) to precision >= da-db+1.  Two Karatsuba multiplications.
// t needs 2*(da+1)+64 slots, ws 8*(da+1)+64, arv/qb each da+1.
static int polremNEWT64(LONG *a,int da,const LONG *B,int db,const LONG *Binv,
                        const LONG p,LONG *arv,LONG *qb,LONG *t,LONG *ws){
    int m,i;
    if( da<db ) {
        while( da>=0 && a[da]==0 ) da--;
        return da;
    }
    m = da-db;                                       // deg of the quotient
    for( i=0; i<=m; i++ ) arv[i] = a[da-i];          // rev(a) mod x^(m+1)
    polmulKARpad64(arv,m,Binv,m,t,p,qb,qb+(m+1),ws); // qrev = rev(a)*Binv
    for( i=0; i<=m; i++ ) arv[i] = t[m-i];           // q = rev(qrev)
    polmulKARpad64(arv,m,B,db,t,p,qb,qb+(da+1),ws);  // q*B
    for( i=0; i<db; i++ ) a[i] = sub64b(a[i],t[i],p);
    da = db-1;
    while( da>=0 && a[da]==0 ) da--;
    return da;
}

// C := A^n mod B, exactly polpowmod64s's contract, for large deg(B).
static int polpowmodKAR64(LONG *A,LONG n,LONG *B,int da,int db,LONG *C,LONG p){
    static std::vector<LONG> Binv,R,W1,W2,W3,W4;
    int dc,k,i,m,b[64];
    LONG hi;

    if( n==0 ) { C[0] = 1; return 0; }
    if( da>=db ) da = poldiv64s(A,B,da,db,p);
    if( da<0 ) { C[0] = 0; return -1; }               // A = 0 mod B

    if( (int)Binv.size() < db )        Binv.resize((size_t)db);
    if( (int)R.size()    < 2*db+2 )    R.resize((size_t)2*db+2);
    if( (int)W1.size()   < 2*db+2 )    W1.resize((size_t)2*db+2);
    if( (int)W2.size()   < 8*db+16 )   W2.resize((size_t)8*db+16);
    if( (int)W3.size()   < 8*db+128 )  W3.resize((size_t)8*db+128);
    if( (int)W4.size()   < 16*db+64 )  W4.resize((size_t)16*db+64);

    polrecipNEWT64(B,db,db,Binv.data(),p,W2.data(),W3.data());
    ...
    while( k>0 ) { k--;
        // C := C^2 mod B
        for( i=0; i<=dc; i++ ) W1[i] = C[i];
        polmulKAR64(W1.data(),W1.data(),R.data(),dc+1,p,W3.data());
        dc = polremNEWT64(R.data(),2*dc,B,db,Binv.data(),p,
                          W1.data(),W2.data(),W4.data(),W3.data());
        for( i=0; i<=dc; i++ ) C[i] = R.data()[i];
        if( b[k]==1 ) {
            if( da<=1 ) {
                // C := (A1 x + A0) * C mod B directly in O(db)
                hi = (dc==db-1 && da==1) ? C[db-1] : 0;
                for( i=dc+da; i>=0; i-- ) {
                    LONG v = 0;
                    if( da==1 && i>=1 && i-1<=dc ) v = mul64b(A[1],C[i-1],p);
                    if( i<=dc ) v = add64b(v,mul64b(A[0],C[i],p),p);
                    R.data()[i] = v;
                }
                dc = dc+da;
                if( dc==db ) {                        // one step of division
                    for( i=0; i<db; i++ )
                        R.data()[i] = sub64b(R.data()[i],mul64b(hi,B[i],p),p);
                    dc = db-1;
                }
                while( dc>=0 && R.data()[dc]==0 ) dc--;
                for( i=0; i<=dc; i++ ) C[i] = R.data()[i];
            } else {
                polmulKARpad64(A,da,C,dc,R.data(),p,
                               W1.data(),W1.data()+db+1,W3.data());
                dc = polremNEWT64(R.data(),da+dc,B,db,Binv.data(),p,
                                  W1.data(),W2.data(),W4.data(),W3.data());
                for( i=0; i<=dc; i++ ) C[i] = R.data()[i];
            }
        }
...
int polpowmod64s( LONG * A, LONG n, LONG * B, int da, int db, LONG *C, LONG *R, LONG p )
{
    ...
    if( n==0 ) { C[0] = 1; return 0; }
    if( db>=POLKARPOW_CUTOFF ) return polpowmodKAR64(A,n,B,da,db,C,p);
