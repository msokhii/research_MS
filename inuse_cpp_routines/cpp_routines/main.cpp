...
/* ============================================================================
 *  FTRFR C++ ROUTINES  --  paste this whole block into main.cpp
 *
 *  Written in the same style as ratReconNormal / ratRECON_C: plain functions
 *  over vector<LONG> / LONG*, driving your existing kernels directly --
 *      polDIVIP64   (division: quotient in the high part, remainder in the low)
 *      polSUBMUL64  (ratReconNormal degree-1 quotient fast path)
 *      pMULIP64, pSUBIP64                       (general t-sequence update)
 *      monic64s, modinv64b, mul64b, mul64bASM, sub64b, neg64s   (scalars/norm)
 *  The only steps ratReconNormal does not template are the polynomial GCD and
 *  the divide-out; those are the two small helpers polGCDvec / polExactQuo.
 *
 *  Paste BELOW the definitions it uses (lowest is polDIVIP64, ~line 835) --
 *  e.g. right after ratReconNormal, or just before the final main().
 *  Coefficient convention: low-to-high, index i = coefficient of x^i.
 * ==========================================================================*/

/* ---- polynomial gcd (monic), built on polDIVIP64 + monic64s -------------- */
static int polGCDvec(std::vector<LONG> a,int degA,std::vector<LONG> b,int degB,LONG p,std::vector<LONG>&g){
    if(degA<degB){ std::swap(a,b); std::swap(degA,degB); }
    while(degB>=0){
        a.resize(degA+1);
        int degR=polDIVIP64(a.data(),b.data(),degA,degB,p);       // a low part := a mod b
        std::vector<LONG> rem;
        if(degR>=0) rem.assign(a.begin(),a.begin()+degR+1); else rem.assign(1,0);
        a=b; degA=degB; b=rem; degB=degR;
    }
    if(degA<0){ g.assign(1,1); return 0; }
    g=a; g.resize(degA+1); monic64s(g.data(),degA,p);
    return degA;
}
/* exact quotient Q := A / B  (B | A). polDIVIP64 leaves the quotient in the
   high part a[degB..degA], exactly as ratReconNormal reads it. */
...
/* ---- one Euclidean step:  rem = Rp - Q*R ,  nT = Tp - Q*T ------------------
   Uses the ratReconNormal degree-1 quotient fast path (polSUBMUL64) when the
   quotient is linear (degQ==1); otherwise the general polDIVIP64 path.        */
...
/* ==========================================================================
   DFTRFR : deterministic fault-tolerant reconstruction.
   returns 1 = ok (num/den filled, den monic), 0 = reconstruction FAIL.
   Requires degM > N + D + 2E.
   ========================================================================== */
static int dftrfr(const std::vector<LONG>&M,const std::vector<LONG>&U,int degM,int degU,
    ...
    { std::vector<LONG> gg; int dg=polGCDvec(M,degM,U,degU,p,gg); if(dg>N+E) return 0; } // gcd test
...
/* ==========================================================================
   HFTRFR : maximal-quotient gap reconstruction.
   fills f, g (den monic), Lambda (bad polynomial), qmax. returns 0.
   Maple takes Roots(Lambda) as the bad set.
   ========================================================================== */
...
/* ---- Maple entry point : DFTRFR  (REF-free) ------------------------------
 *  Return code carries the status (REF scalar write-back is unreliable here,
 *  the same reason cppRR recomputes degNOUT):
 *      0 = success (nOut/dOut hold num/den, den monic)
 *      1 = reconstruction FAILED
 *      negative = bad arguments
 *  Maple recovers deg(num)/deg(den) from the arrays via checkZeroPY.
 */
...
/* ---- Maple entry point : HFTRFR  (REF-free) ------------------------------
 *  fOut/gOut : recovered numerator/denominator (den monic)
 *  lamOut    : bad polynomial Lambda ; Maple computes Roots(Lambda) = bad set
 *  info      : length >= 1 ; info[0] = qmax (the gap). Passed as an ARRAY,
 *              not a REF, so Maple reads it back reliably.
 *  Maple recovers deg(f)/deg(g)/deg(Lambda) from the arrays via checkZeroPY.
 *  Return code : 0 = ok, negative = bad arguments.
 */
