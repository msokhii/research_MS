...
/* ========================================================================= */
/* cppGenMono : monomial recovery for discrete-log Ben-Or/Tiwari.            */
/*                                                                          */
/* Setting.  p is prime with  p-1 = 2*c*q_1*...*q_n,  the q_j pairwise      */
/* coprime (distinct primes), and omega_j in GF(p)* has order exactly q_j.  */
/* MRFI samples at sigma_j = (omega_1^j,...,omega_n^j), so each root of the */
/* Berlekamp-Massey generator is  r = prod_j omega_j^{e_j} mod p  for one   */
/* monomial  prod_j x_j^{e_j}  with  0 <= e_j < q_j.                        */
/*                                                                          */
/* Recovery.  Let N_j = prod_{k<>j} q_k (taken mod p-1).  For k <> j the    */
/* order q_k of omega_k divides N_j, so                                     */
/*        r^{N_j} = omega_j^{e_j N_j} = w_j^{e_j},   w_j := omega_j^{N_j},  */
/* and w_j has order q_j because gcd(q_j,N_j)=1.  Hence e_j is a discrete   */
/* log in a cyclic group of tiny order q_j: a q_j-entry lookup table.       */
/* Every quantity stays in [0,p) -- no wraparound for any degree, unlike    */
/* trial division by prime evaluation points, which needs the *integer*     */
/* prod pr_j^{e_j} < p and fails as soon as it exceeds p.                   */
/*                                                                          */
/* The final check  prod_j omega_j^{e_j} == r  rejects any r with a         */
/* component in the cofactor subgroup of order 2c (such r is not a          */
/* monomial value).  NOTE: if a true exponent satisfies e_j >= q_j, it is   */
/* aliased to e_j mod q_j and this check CANNOT see it; the caller must     */
/* pick q_j > deg_{x_j} and verify the assembled result (MRFI does both).   */
/*                                                                          */
/* Return: 0 = OK (expOut[i*n+j] = e_j of root i, row-major T x n),         */
/*         1 = FAIL (zero root, no exponent match, or cofactor component),  */
/*        <0 = bad arguments / omega_j lacks order q_j.                     */
/* Cost:  O(sum_j q_j) table build + O(T*n*log p) mulmods.                  */
/* Requires p < 2^62 (add64b/mul64b conventions of this file).              */
/* ========================================================================= */

/* powmod64s above is only sound when every bit of n is 1 (it skips the     */
/* squaring on zero bits); it is used in this file solely with n = shift = 1. */
/* cppGenMono needs arbitrary exponents, so it uses its own square-and-      */
/* multiply.                                                                 */
static inline LONG powmodGM64(LONG a, LONG n, LONG p){
    LONG r=1, s=a%p; if(s<0) s+=p;
    while(n>0){
        if(n&1) r=mul64b(r,s,p);
        s=mul64b(s,s,p);
        n>>=1;
    }
    return r;
...
extern "C" int cppGenMono(int T, const LONG *roots,
                          int n, const LONG *omega, const LONG *q,
                          const LONG p,
                          int outLen, LONG *expOut)
{
    if(!roots||!omega||!q||!expOut)               return -1;
    if(T<1||n<1)                                  return -1;
    if((long long)outLen < (long long)T*n)        return -1;
    if(p<3)                                       return -1;
    const LONG pm1 = p-1;

    /* N_j = (prod_{k<>j} q_k) mod (p-1); reduction valid by Fermat. */
    std::vector<LONG> Nj(n);
    for(int j=0;j<n;j++){
        LONG acc=1%pm1;
        for(int k=0;k<n;k++){
            if(k==j) continue;
            LONG qk=q[k]%pm1; if(qk<0) qk+=pm1;
            acc=mul64b(acc,qk,pm1);
        }
        Nj[j]=acc;
    }

    /* Subgroup tables: w_j = omega_j^{N_j}, order q_j; store w_j^t -> t. */
    std::vector<std::unordered_map<LONG,int>> tab(n);
    for(int j=0;j<n;j++){
        if(q[j]<1||q[j]>=p) return -1;
        LONG wj=powmodGM64(omega[j],Nj[j],p);
        if(q[j]>1 && wj==1) return -2;        /* omega_j lacks order q_j     */
        tab[j].reserve((size_t)(2*q[j]));
        LONG v=1;
        for(LONG t=0;t<q[j];t++){
            if(!tab[j].emplace(v,(int)t).second) return -2; /* ord(w_j)<q_j */
            v=mul64b(v,wj,p);
        }
        if(v!=1) return -2;                   /* ord(w_j) does not divide q_j */
    }

    for(int i=0;i<T;i++){
        LONG r=roots[i]%p; if(r<0) r+=p;
        if(r==0) return 1;                    /* 0 is never a monomial value */
        LONG check=1;
        for(int j=0;j<n;j++){
            LONG v=powmodGM64(r,Nj[j],p);
            auto it=tab[j].find(v);
            if(it==tab[j].end()) return 1;    /* no exponent < q_j matches   */
            LONG e=(LONG)it->second;
            expOut[(size_t)i*n+j]=e;
            check=mul64b(check,powmodGM64(omega[j],e,p),p);
        }
        if(check!=r) return 1;                /* cofactor component present  */
    }
    return 0;
}
